      ...
      >
        <el-dropdown slot="prepend" placement="bottom" @command="changedata">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item v-for="each_type in sctypelist" :key="each_type" :command="each_type">
...
export default {
  ...
  methods: {
    ...
    handleSelect: function(item) {
      ...
      this.closePopler();
      let link = this.scdata.url + "?" + this.scdata.key + "=" + item.value;
      window.open(link);
    },
    // 关闭推荐词下拉框
    closePopler: function() {
      var dom = document.getElementsByClassName("suggestion-word-popper")[0];
